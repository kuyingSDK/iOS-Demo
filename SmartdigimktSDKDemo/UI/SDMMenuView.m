...
#import "SDMSelectViewController.h"
#import <Masonry/Masonry.h>
#import "SDMDemoUIHeader.h"

@interface SDMMenuView()
...
- (void)setupUI
{
    ...
    if (!self.subMenuList) {
        ...
        [self.menu mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(self.mas_bottom).offset(kScaleW(-56));
            make.left.equalTo(self.mas_left).offset(kScaleW(22));
            make.width.mas_equalTo((kScreenW - 80) * 0.5);
            make.height.mas_equalTo(kScaleW(64));
        }];
...
- (void)resetMenuList:(NSArray *)menuList subMenuList:(NSArray *)subMenuList {
    _menuList = menuList;
    _subMenuList = subMenuList;
    [_menu setTitle:_menuList.firstObject forState:UIControlStateNormal];
    [_subMenu setTitle:_subMenuList.firstObject forState:UIControlStateNormal];
}
...
- (void)resetSubMenuList:(NSArray *)subMenuList index:(NSUInteger)index {
    _subMenuList = subMenuList;
    if (index < _subMenuList.count) {
        [_subMenu setTitle:_subMenuList[index] forState:UIControlStateNormal];
    } else {
        [_subMenu setTitle:_subMenuList.firstObject forState:UIControlStateNormal];
    }
}
...
- (void)showMenuList
{
    SDMSelectViewController *vc = [[SDMSelectViewController alloc] initWithSelectList:self.menuList multipleSelection:self.multiple];
    ...
    [vc setSelectBlock:^(NSArray * _Nonnull select) {
        ...
        [weakSelf.autoSwitch setOn:NO];// = false;
        if (self.turnOnAuto) {
            self.turnOnAuto(NO);
        }
...
{
    SDMSelectViewController *vc = [[SDMSelectViewController alloc] initWithSelectList:self.subMenuList];
    __weak typeof(self) weakSelf = self;
    [vc setSelectBlock:^(NSArray * _Nonnull select) {
